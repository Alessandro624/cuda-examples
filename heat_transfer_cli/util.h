    ...
    private:
        ...
        uint64_t getTime(unsigned long long scale)
        {
            ...
            if ((tv.tv_usec - startTime_.tv_usec) < 0)
            {
                ...
                // Then scale.
                ticks = (uint64_t)(tv.tv_sec - startTime_.tv_sec - 1) * scale + (uint64_t)((1000ULL * 1000ULL) + tv.tv_usec - startTime_.tv_usec) * scale / (1000ULL * 1000ULL);
            }
            else
            {
                ticks = (uint64_t)(tv.tv_sec - startTime_.tv_sec) * scale + (uint64_t)(tv.tv_usec - startTime_.tv_usec) * scale / (1000ULL * 1000ULL);
            }
#else
            struct timespec tp;
            ...
            if ((tp.tv_nsec - startTime_.tv_nsec) < 0)
            {
                ...
                // Then scale
                ticks = (uint64_t)(tp.tv_sec - startTime_.tv_sec - 1) * scale + (uint64_t)((1000ULL * 1000ULL * 1000ULL) + tp.tv_nsec - startTime_.tv_nsec) * scale / (1000ULL * 1000ULL * 1000ULL);
            }
            else
            {
                ticks = (uint64_t)(tp.tv_sec - startTime_.tv_sec) * scale + (uint64_t)(tp.tv_nsec - startTime_.tv_nsec) * scale / (1000ULL * 1000ULL * 1000ULL);
            }
#endif //_WIN32
